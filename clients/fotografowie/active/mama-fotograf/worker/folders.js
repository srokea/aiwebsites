 ...
 * ON DELETE CASCADE), bo i tak musimy zebrać r2_key, żeby wyczyścić pliki z R2.
 */
...
export async function deleteFolder(env, id) {
  ...
  const { results } = await env.DB
    .prepare(`SELECT items.r2_key FROM items
              JOIN albums ON albums.id = items.album_id
              WHERE albums.folder_id = ? AND items.type = 'photo'`)
    .bind(id)
    .all();
