# Mama Fotograf — Magda Suszka-Krawiec Photography

Instancja portfolio dla tej klientki, sklonowana z `clients/fotografowie/portfolio-template`.
Backend (`worker/`, `schema.sql`) i panel (`public/admin/`) są identyczne z szablonem.
Bespoke jest tylko `public/index.html`, `public/privacy.html` i `public/assets/`.

**Do uzupełnienia przed wdrożeniem:**
- `API_BASE` w `public/index.html` i `public/admin/index.html` (krok 7 niżej),
- `ALLOWED_ORIGIN`, `R2_PUBLIC_URL` i `database_id` w `wrangler.toml`,
- adres domeny w `public/index.html`: `og:url`, `og:image`, `link rel="canonical"` i `url`/`image` w JSON-LD (wszędzie `DOMENA_KLIENTA.pages.dev`).

**Czeka na dane od klientki:** tekst „O mnie", cennik, zdjęcia portfolio
(wszystkie trzy miejsca są na stronie oznaczone kafelkami z przerywaną ramką).
Zdjęcia portfolio Magda wgrywa sama w panelu `/admin/`.

Instrukcja deploymentu szablonu (bez zmian):
