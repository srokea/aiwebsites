/**
 * worker/items.js — kafelki w siatce albumu: zdjęcia, rolki wideo i bloki tekstu.
 *
 ...
 * - Rolka (type: 'video') to DWA obiekty w R2: sam plik wideo i klatka-plakat.
 *   Plakat też wycina przeglądarka, z pierwszej klatki filmu.
...
// Rolka jest znacznie cięższa niż zdjęcie. 60 MB mieści się w limicie
// requestu Workers i spokojnie starcza na pionowy film do minuty.
...
export function toPublicItem(env, row) {
  ...
  if (row.type === 'video') {
    return {
      ...
      poster: row.poster_key ? photoUrl(env, row.poster_key) : null,
...
/**
 * Upload rolki. FormData: `file` (wideo), `poster` (WebP, klatka z filmu),
 * `albumId`, opcjonalnie `caption`.
 * Klucze w R2: {folderId}/{itemId}.mp4 oraz {folderId}/{itemId}-poster.webp
 *
 * Kafelek rolki jest domyślnie wąski i wysoki (2x4), bo materiał jest pionowy 9:16.
 */
export async function uploadVideo(env, formData) {
  const file = formData.get('file');
  const poster = formData.get('poster');

  if (!file || typeof file === 'string') throw new Error('Brak pliku w polu `file`');
  if (file.size > MAX_VIDEO_BYTES) throw new Error('Rolka jest za duża (limit 60 MB)');

  const album = await findAlbum(env, formData.get('albumId'));
  if (!album) throw new Error('Album nie istnieje');

  const id = crypto.randomUUID();
  const extension = (file.name || '').toLowerCase().endsWith('.webm') ? 'webm' : 'mp4';
  const r2Key = `${album.folder_id}/${id}.${extension}`;
  const posterKey = poster && typeof poster !== 'string'
    ? `${album.folder_id}/${id}-poster.webp`
    : null;

  // Najpierw pliki do R2 — jak się nie uda, w bazie nie zostanie sierota.
  await env.BUCKET.put(r2Key, file.stream(), {
    httpMetadata: {
  ...
  const filename = (file.name || 'rolka.mp4').slice(0, 200);
  const caption = String(formData.get('caption') || '').trim().slice(0, 200)
    || filename.replace(/[.][^.]+$/, '');

  try {
    await env.DB
      .prepare(`INSERT INTO items (id, album_id, type, grid_w, grid_h, position, filename, r2_key, poster_key, display_name)
                VALUES (?, ?, 'video', 2, 4, ?, ?, ?, ?, ?)`)
      .bind(id, album.id, await nextPosition(env, album.id), filename, r2Key, posterKey, caption)
      .run();
  } catch (error) {
    // Zapis metadanych padł — sprzątamy pliki, żeby nie zostały śmieci w R2.
    await deleteR2Keys(env, [r2Key, posterKey]);
    throw error;
  }
...
/** Usuwa kafelek (dla zdjęcia i rolki także pliki z R2). */
export async function deleteItem(env, id) {
  const row = await env.DB
    .prepare('SELECT r2_key, poster_key FROM items WHERE id = ?')
    .bind(id)
    .first();
  if (!row) return false;

  await deleteR2Keys(env, [row.r2_key, row.poster_key]);
