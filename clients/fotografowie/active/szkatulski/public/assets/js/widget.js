...
(function () {
  ...
  const WIDGET_CONFIG = {
    // TODO: uzupełnić 669 889 133 i filip_szkatulski@onet.pl razem z index.html.
    channels: {
      phone: 'tel:+48669889133',
      instagram: 'https://ig.me/m/szkatulski_photography',
      messenger: 'https://m.me/61569980133400',
      email: 'mailto:filip_szkatulski@onet.pl',
    },

    // Godziny, w których przy przycisku świeci się kropka "dostępny".
    // 0 = niedziela ... 6 = sobota, zgodnie z Date#getDay(). Realny grafik:
    // pon-sob 9-22, niedziela 10-22.
    availability: {
      0: { from: 10, to: 22 },
      1: { from: 9, to: 22 },
      2: { from: 9, to: 22 },
    ...
    faq: [
      {
        q: 'Ile kosztuje zlecenie?',
        a: 'Zależy od długości i zakresu. 18stka albo event to zwykle 600 do 2000 zł, '
         + 'teledysk 600 do 1000 zł. Sport i auta wyceniam indywidualnie. Napisz, co planujesz.',
      },
      {
        q: 'Jak szybko dostanę zdjęcia?',
        a: 'Standardowo paczka jest u Ciebie w 3 dni od wydarzenia. Jak potrzebujesz kilku '
         + 'zdjęć na już, wybrane kadry wysyłam nawet tego samego wieczoru.',
      },
      {
        q: 'Jak daleko dojeżdżasz?',
        a: 'Bazuję w Sulejowie, normalnie jeżdżę po całym woj. łódzkim. Dalej też dam radę, '
         + 'dojazd doliczam do wyceny.',
      },
      {
        q: 'Robisz też wideo?',
        a: 'Tak, nagrywam i montuję sam. Rolki, teledyski do 6 minut, content na social media. '
         + 'Dostajesz materiał gotowy do wrzucenia.',
      },
      {
        q: 'Jak zarezerwować termin?',
        a: 'Napisz albo zadzwoń, ustalamy szczegóły i termin jest Twój. '
         + 'Przy większych zleceniach biorę zaliczkę.',
      },
      {
        q: 'Czym pracujesz?',
        a: 'Canon R8, kilka obiektywów, gimbal DJI, oświetlenie jak trzeba. Montuję w Premiere.',
      },
    ],
  };
  ...
  async function sendMessage(text) {
    ...
    return hit ? hit.a : 'Napisz do mnie na Instagramie albo zadzwoń, odpiszę osobiście.';
  }

  // Widget siedzi we własnym zakresie, więc punkt wejścia wystawiamy jawnie.
  window.SzkatulskiWidget = { config: WIDGET_CONFIG, sendMessage };
  ...
  const SEEN_KEY = 'szk_widget_seen';
  ...
  function makeButton() {
    ...
    if (isAvailable()) {
      ...
      dot.innerHTML = '<span class="sr-only">Filip jest teraz dostępny</span>';
      button.append(dot);
    }
    return button;
  }
  ...
  function makePanel() {
    ...
    panel.setAttribute('aria-label', 'Kontakt z Filipem');
    ...
    panel.innerHTML =
      ...
      +   '<img class="widget-avatar" src="assets/img/avatar-filip.webp" alt="" aria-hidden="true">'
      +   '<div>'
      +     '<div class="widget-head__name">Filip Szkatulski</div>'
      +     '<div class="widget-head__sub">Zwykle odpowiadam w kilka godzin</div>'
      +   '</div>'
      ...
      +   '<span>Nie ma Twojego pytania?</span>'
  ...
  if (!seen && !reduce) {
    ...
    nudge.type = 'button';
    nudge.textContent = 'Masz pytanie? Napisz śmiało';
