/**
 * POST /api/reserve — zapis rezerwacji stolika.
 *
 ...
 *
 * Odpowiedź: { ok: true, reservation } albo { ok: false, error: '<kod>' }.
 * Kody błędów obsługuje js/booking.js (funkcja errorMessage).
 */
...
export function parseInput(body) {
  ...
  const e = optionalText(email, 254);
  if (!e.ok || (e.value && !EMAIL_RE.test(e.value))) return null;
  ...
  return {
    ...
    name: cleanName, phone: cleanPhone, email: e.value, comment: c.value,
    turnstileToken,
  };
}
...
export async function onRequestPost({ request, env }) {
  ...
  if (result.ok) {
    return reply(200, { ok: true, reservation: result.reservation });
  }
