...
> **Wyjątek w obrębie puli ziemistej (Zajawa Barbershop, 2026-07-28):** ciepły taupe/drewno `#9C8265` — kolor drewnianej podłogi w salonie, wskazany wprost w briefie klienta jako `#8B7355 lub podobny`. Rozjaśniony do `#9C8265`, bo `#8B7355` daje tylko 4,21:1 na `#111111` i nie przechodzi WCAG AA jako mały tekst (etykiety sekcji, gwiazdki). To ten sam ton, nie nowy kolor — mieści się w puli ziemistej, więc rozszerza ją o potwierdzony odcień taupe zamiast być wyjątkiem poza pulą. Tło u tego klienta to `#0A0A0A`/`#111111` (brief), nie `#0B0906`/`#232323`.
